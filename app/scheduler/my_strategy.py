...
from app.utils.policy_ind_config import PolicyIndConfig, ind_name_relation
from app.utils.strategy_unified import StrategyUnified
...
class MyStrategy:
    ...
    def run(self):
        ...
        for config in run_strategy_config:
            ...
            cerebro = bt.Cerebro()
            # Add a strategy
            batch_no = config.start_date + "_" + config.end_date + "_" + config.stock_code + "".join(
                [str(random.randint(0, 10)) for i in range(4)])
            batch_no = batch_no.replace("-", "")
            cerebro.addstrategy(StrategyUnified, batch_no=batch_no, params=config.params)
            ...
            # Run over everything
            result = cerebro.run(maxcpus=4)[0]
            # Print out the final result
            final_capital = cerebro.broker.getvalue()
            print('Final Portfolio Value: %.2f' % final_capital)

            #
            # Plot the result
            # cerebro.plot()
            self.dao.update_config(config.id, result.batch_no, final_capital)
            self.dao.all_save_log(result.trader_log)


class StrategyUnified(bt.Strategy):
    # 默认值 必须给
    params = (
        ("dpo_period", 20),
        ("er_period_me1", 20),
    ...
    def __init__(self, *args, **kwargs):
        self.trader_log = []
        self.ind_params = json.loads(kwargs["params"])
        # 绑定对应关系
        for key, value in ind_name_relation.items():
            ...
            if config_params_value is not None:
                setattr(self.p, key, config_params_value[index])
    ...
    def notify(self, order):
        ...
        if order.status in [order.Completed, order.Canceled, order.Margin]:
            ...
            self.log('SELL 交易金额, %.2f' % (abs(order.executed.size) * order.executed.price))
            stock_strategy_log2 = StockStrategyLog2(batch_no=self.batch_no,
                                                    trans_fee=order.executed.comm,
                                                    trans_date=self.datas[0].datetime.date(0).isoformat(),
                                                    mark_price=order.executed.price,
                                                    trans_amount=abs(order.executed.size) * order.executed.price,
                                                    trans_size=abs(order.executed.size))
            if order.isbuy():
